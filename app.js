...
(function () {
  ...
  const SEED_REVIEWS = (() => {
    ...
    const wText = [
      'Делала перманент бровей — результат превзошёл ожидания! Натурально, аккуратно и совсем не больно.',
      'Губы зажили идеально, цвет именно такой, как хотела. Алёна — настоящий профессионал.',
      'Подобрали форму бровей под лицо, теперь утром не нужно ничего подкрашивать. Очень довольна!',
      'Спасибо за натуральные брови! Никто не верит, что это перманент.',
      'Сделала натуральный перманент губ — нежный цвет, всё аккуратно. Рекомендую!',
      'Очень внимательный мастер, всё объяснила и подобрала оттенок. Результатом в восторге.',
      'Брови выглядят как свои, только ухоженные. Зажило быстро, уход подробно расписали.',
      'Давно хотела перманент губ — наконец решилась и не пожалела. Красиво и естественно.',
      'Стерильно, комфортно, без боли. Цвет бровей идеально подошёл к волосам.',
      'Пришла на коррекцию — освежили цвет, форма держится отлично. Спасибо!',
      'Лучший мастер! Брови ровные, симметричные, цвет натуральный. Буду советовать подругам.',
      'Губы выглядят сочно, но естественно. Очень довольна работой и отношением.',
      'Перманент бровей держится больше года, цвет красиво выцветает. На коррекцию только сюда.',
      'Аккуратная работа, приятная атмосфера. Результат именно то, что хотела.',
      'Сделала перманент впервые, очень боялась — зря! Всё прошло легко, результат шикарный.',
    ];
    const mText = [
      'Делал трихопигментацию — зоны поредения стали незаметны, выглядит очень естественно. Спасибо!',
      'Отличный результат, волосы визуально гуще. Никто не догадывается, что это процедура.',
      'Подкорректировали форму бровей — стали аккуратнее и естественнее. Доволен результатом.',
      'Профессиональный подход, всё стерильно. Трихопигментацией очень доволен.',
      'Замаскировали залысины, смотрится натурально. Рекомендую!',
      'Делал брови — аккуратно, без эффекта нарисованных. Спасибо за работу!',
      'Результат превзошёл ожидания, голова выглядит ухоженно. Всё быстро и комфортно.',
      'Качественно и аккуратно. Трихопигментация реально работает, выгляжу моложе.',
      'Спасибо за профессионализм! Брови теперь ровные и естественные.',
      'Отличный мастер, подробно рассказали про уход. Доволен на все сто.',
    ];
    ...
    for (let i = 0; i < 215; i++) {
      ...
      if (isMale) {
        ...
        text = mText[(m * 3) % mText.length];
        m++;
      } else {
        const f = i - m;
        name = female[f % female.length];
        text = wText[(f * 5 + 2) % wText.length];
      }
  ...
  function bind() {
    ...
    $('#cartCheckout').addEventListener('click', () => {
      if (!state.cart.length) return toast('Список записи пуст');
      closeCart();
      $('#footer').scrollIntoView({ behavior: 'smooth' });
      toast('Заполните форму записи — я свяжусь с вами');
    });
    // modal
